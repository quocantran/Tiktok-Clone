...
const Comment = () => {
  ...
  useEffect(() => {
    if (!closeModal) {
      commentWrapperMobile.current = document.getElementById(
    ...
    if (commentWrapperMobile.current) {
      ...
      return () => {
        commentWrapperMobile.current?.removeEventListener(
          "scroll",
          handleScroll
        );
      };
      ...
      <div className={cx("video-container")} {...handlers}>
        <div ref={videoPlayerRef} className={cx("video-player")}>
          ...
          <div onClick={handleClick} className={cx("video-space")}>
            ...
            ></video>
            {loading ? (
              <Loading className={cx("comment-video-loading")} />
            ) : (
              Fragment
            )}
          </div>
        ...
        >
          ...
          >
            <div
              ...
              className={cx("close-icon-mobile")}
            >
              <FontAwesomeIcon icon={faClose} />
            </div>
            <header
              className={cx("comment-header-mobile")}
            >{`${data.comments_count} bình luận`}</header>
